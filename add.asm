segment .data
	...
	total TIMES 20 db "0"		; total de cong tong cua hai so 
	
segment .bss
	num1 resb 21				;so dau tien
	num2 resb 21				; so thu 2
	num1_rev resb 20			; dung de chua so dau tien bi dao nguoc
	num2_rev resb 20			; dung de chua so thu 2 bi dao nguoc
segment .text
	global _start:
_start:
	...
	dec r8
	mov r9,0					; set r9=0
	mov ecx,20					; dung ecx lam bien counter cho vong lap _rev_num1
...
_rev_num2:
	...
	mov ecx,20					; dung thanh ecx lam counter loop cho add_loop
	mov r8,0					;r8=0
	mov r9,19					;r9=19 chi den gia tri tan cung ben phai cua total
	mov sil,0					; sil =0
	clc
_add_loop:
	...
	pushf						; cho nay em hoc tren tutorial point 
	or al,0x30
	popf
